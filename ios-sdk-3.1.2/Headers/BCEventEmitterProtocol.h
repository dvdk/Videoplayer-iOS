 ...
 *
 * @warning The caller of this method is responsible for ensuring that 
 * [BCEventEmitterProtocol off:removeListener:] is called before any of the 
 * objects referenced in block are deallocated.
 *
 * @warning Once the listeners for a given event begin, all listeners will run,
 * so it is not possible to use [BCEventEmitterProtocol off:removeListener:] to
 * prevent a registered listener from executing. Do not invalidate application
 * state needed by multiple listeners until all listeners who rely on that state
 * have finished executing. For example, if two event listeners are registered
 * for `BCEventVideoDidEnd` and the first listener deallocates an object needed
 * in the second listener, an EXC_BAD_ACCESS will occur.
 * 
